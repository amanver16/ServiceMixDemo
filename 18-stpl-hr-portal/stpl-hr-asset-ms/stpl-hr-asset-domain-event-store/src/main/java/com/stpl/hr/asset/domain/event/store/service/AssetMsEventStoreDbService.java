package com.stpl.hr.asset.domain.event.store.service;
...
public class AssetMsEventStoreDbService {

	public AssetMsEventStoreDbService() {
		super();
	}
	...
	public int checkForDuplicateAggregate(String aggregateId) throws SQLException {
		String duplicateCheckQuery = AssetMsEventStoreConstant.GET_AGGREGATE_COUNT;
		int returnValue = 0;
	...
	public void addAggregate(AssetMsGenericEvent aggregateData) {
		String insertQuery = AssetMsEventStoreConstant.INSERT_AGGREGATE;
		try (Connection connection = eventDataSource.getConnection();
				PreparedStatement insertStatement = connection.prepareStatement(insertQuery)) {
			insertStatement.setString(1, aggregateData.getAggregateId());
			insertStatement.setString(2, AssetMsEventStoreConstant.ASSET);
	...
	public void addAggregateEvent(AssetMsGenericEvent aggregateEventData, String content) throws SQLException {
		String insertQuery = AssetMsEventStoreConstant.INSERT_AGGREGATE_EVENT;
		try (Connection connection = eventDataSource.getConnection();
				PreparedStatement insertStatement = connection.prepareStatement(insertQuery)) {
	...
	private AssetMsAggregateDao getAggregate(String aggregateId) {
		...
		String getAggregateQuery = AssetMsEventStoreConstant.GET_AGGREGATE_DATA;
		AssetMsAggregateDao aggregate = null;
		try (Connection connection = eventDataSource.getConnection();
	...
	private void getAggregateEvents(String aggregateId, AssetMsAggregateDao aggregate) throws SQLException {
		...
		String getAggregateEventQuery = AssetMsEventStoreConstant.GET_AGGREGATE_EVENT_DATA;
